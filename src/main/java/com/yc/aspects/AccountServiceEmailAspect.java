...
public class AccountServiceEmailAspect {
    ...
    //存款
    @Pointcut("execution(* com.yc.service.AccountService.deposit(..))") // 所有以update开头的方法
    public void deposit() {}

    //取款
    @Pointcut("execution(* com.yc.service.AccountService.withdraw(..))") // 所有以update开头的方法
    public void withdraw() {}
    //转账
    @Pointcut("execution(* com.yc.service.AccountService.transfer(..))") // 所有以update开头的方法
    ...
    public void sendEmail(  JoinPoint jp ) {  //连接点 即目标方法的参数
        ...
        Accounts account = service.findAccount(  accountid );
...
//            mailBiz.sendMail(email,"账户变动通知",info);
    ...
    // 生成存款信息
    private String depositInfo( Accounts account ,double money){
        Date d = new Date();
    ...
    // 生成取款信息
    private String withdrawInfo( Accounts account ,double money){
        Date date = new Date();
    ...
    // 生成存款信息
    private String transferInfo( Accounts account ,double money , int toAccountid){
        Date date = new Date();
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String time = df.format(date);
